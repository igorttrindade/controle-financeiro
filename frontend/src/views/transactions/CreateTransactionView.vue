  ...
  <div class="min-h-screen bg-base-200 text-base-content">
    ...
    <main class="mx-auto w-full max-w-4xl px-4 pb-10 pt-24 md:px-8">
      <div class="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-3xl font-bold">Nova transação</h1>
          <p class="text-sm text-base-content/70">Registre uma entrada ou despesa com operação, valor e descrição.</p>
        </div>
      ...
      <div v-if="isLoadingOperations" class="card border border-base-300 bg-base-100 shadow-card">
...
import { createOperation, createTransaction, getOperations } from '@/services/transactions/transaction.service'
...
async function loadOperations() {
  ...
  try {
    ...
    operations.value = Array.isArray(response) ? response : []
  } catch (error) {
    if (handleUnauthorized(error)) return
    loadError.value = 'Não foi possível carregar as operações. Tente novamente.'
  } finally {
...
async function handleSubmit() {
  ...
  try {
    ...
    if (responseMessage) {
      addNotification(responseMessage, 'error')
    } else {
      addNotification('Não foi possível criar a transação. Tente novamente.', 'error')
    }
  } finally {
